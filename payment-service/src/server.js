...
const { startConsumer, close: closeConsumer } = require('./consumer');
...
app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerSpec, { customSiteTitle: 'SmartCart Payment Service API' }));
...
async function shutdown(signal) {
  ...
  server.close(async () => { await closeConsumer(); await publisher.close(); await db.end(); logger.info('Shutdown complete'); process.exit(0); });
}
