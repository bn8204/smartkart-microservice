...
app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerSpec, { customSiteTitle: 'SmartCart Order Service API' }));
...
async function shutdown(signal) {
  ...
  server.close(async () => { await publisher.close(); await closeConsumer(); await db.end(); logger.info('Shutdown complete'); process.exit(0); });
}
